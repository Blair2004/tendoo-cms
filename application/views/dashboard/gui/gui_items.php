<?php
	...
	{
		$action	=	site_url( array( 'dashboard' , 'options' , 'save' ) );
	}
	if( $form_wrap )
	{
		/**
	...
	if( is_array( $content ) )
	{
		foreach( $content as $item )
		{
			...
			{
				$placeholder 	= riake( 'placeholder' , $item );
				$label			= riake( 'label' , $item );
				$name			= riake( 'name' , $item );
				$value			= riake( 'value' , $item );			
				?>
...
<?php echo $description;?>
	...
	$submit_text	=	riake( 'submit_text' , $form_wrap );
	$reset_text		=	riake( 'reset_text' , $form_wrap );
