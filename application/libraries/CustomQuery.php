...
class CustomQuery
{
	...
	private function title_checker($title, $exclude_post = 0)
	{
		...
		while($matching_post = $this->get(array('where' => array('title' => $title,
																 'id !=' => $exclude_post
		)
										  )))
		{
	...
	public function get_post_legacy($post_id)
	{
		...
		$post_child = $this->get(array('where' => array('parent_id' => $post_id)
								 ));
	...
	{
		$custom = $this->get(array('where' => array('id' => $id)
							 ));
	...
	function custom_query_exists($identifier, $filter = 'as_title')
	{
		...
		return $this->get(array('where' => array($_filter,
												 $identifier
		)
						  ));
	}
	...
	{
		$query = $this->get(array('where' => array('taxonomy_id' => $id)
							));
	...
	function post_comment($post_id, $content, $author = FALSE, $mode = 'create', $comment_id = NULL, $author_name = '', $author_email = '', $reply_to = FALSE)
	{
		...
		$post = $this->get(array('where' => array('id' => $post_id)
						   ));

		if($post) // if it doesn't return an empty array
		{
			$comment_array = array('QUERY_NAMESPACE' => $this->query_namespace,
								   'COMMENTS'        => $content,
								   'DATE'            => $this->datetime,
			);
