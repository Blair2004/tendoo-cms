...
class tendoo_contents_admin_controller extends Libraries
{
	public function __construct($data)
	{
		...
		$this->tendoo_admin->menuExtendsBefore($this->load->view(MODULES_DIR.$this->opened_module['ENCRYPTED_DIR'].'/views/menu',$this->data,true,TRUE));
	...
	public function index($page	=	1)
	{
		...
		$this->data['paginate']	=	$this->tendoo->paginate(10,$this->data['ttFiles'],1,'on','off',$page,$this->url->site_url(array('admin','open','modules',$this->opened_module['ID'],'index')).'/',$ajaxis_link=null);
	...
	public function ajax($x = '',$y = 1,$z = '')
	{
		...
		{
			$this->data['ttFiles']		=	$this->file_contentAdmin->countUploadedFiles();
			$this->data['paginate']		=	$this->tendoo->paginate(12,$this->data['ttFiles'],1,'on','off',$y,$this->url->site_url(array('admin','open','modules',$this->opened_module['ID'],'ajax','selection')).'/',$ajaxis_link=null);
			$this->data['files']		=	$this->file_contentAdmin->getUploadedFiles($this->data['paginate'][1],$this->data['paginate'][2]);
	...
	public function manage($id)
	{
		...
		if($this->input->post('overwrite_file'))
		{
			...
			if($this->form_validation->run())
			{
				...
				if($query)
				{
					$this->notice->push_notice(fetch_error('done'));
				}
				else
				{
					$this->notice->push_notice(fetch_error('error_occured'));
		...
		if($this->input->post('change_file'))
		{
			...
			if($this->form_validation->run())
			{
				...
				$this->notice->push_notice(fetch_error($query));
			}
		}
