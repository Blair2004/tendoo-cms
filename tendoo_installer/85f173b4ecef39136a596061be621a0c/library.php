...
class modus_lib
{
	...
	{
		$query	=	$this->core->db->get('Tendoo_theme_modus_table');
		$result	=	$query->result_array();
		if(count($result) == 0)
		{
			if($this->core->db->insert('Tendoo_theme_modus_table',array(
		...
		{
			if($this->core->db->update('Tendoo_theme_modus_table',array(
				'FACEBOOK_ACCOUNT'		=>		$facebook,
				'TWITTER_ACCOUNT'		=>		$twitter,
				'GOOGLEPLUS_ACCOUNT'	=>		$googleplus
			)))
