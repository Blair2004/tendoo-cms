...
class page_library extends Libraries
{
	function __construct()
	{
		parent::__construct();
		__extends($this);
		$this->module 	=	get_modules( 'pages_editor' , 'filter_namespace' );
		$this->_before_boot();
	}
	...
	private function _file( $action = 'create' , $content = '')
	{
		if( $action == 'create' ){
			...
			file_put_contents( $this->module[0][ 'URI_PATH' ] . 'created_pages/' . $encrypted_name , $content );
			return $encrypted_name;
		}
		else if ( $action == 'get' ){
			if( is_file( $this->module[0][ 'URI_PATH' ] . 'created_pages/' . $content ) ){
				return file_get_contents( $this->module[0][ 'URI_PATH' ] . 'created_pages/' . $content );
			}
			return false;
		}
		else if ( $action == 'delete' ){
			if( is_file( $this->module[0][ 'URI_PATH' ] . 'created_pages/' . $content ) ){
				return unlink( $this->module[0][ 'URI_PATH' ] . 'created_pages/' . $content );
			}
			return false;
		}
	}
	...
	{
		if( !is_dir( $this->module[0][ 'URI_PATH' ] . '/created_pages' ) ){
			mkdir( $this->module[0][ 'URI_PATH' ] . '/created_pages' );
		}
	}
	/**
	...
	{
		$controllers	=	$this->tendoo->getControllersAttachedToModule( $this->module[0][ 'NAMESPACE' ] );
