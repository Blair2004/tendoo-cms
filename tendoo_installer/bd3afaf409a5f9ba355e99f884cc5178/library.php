<?php
	class widhandler_lib
	{
		...
		{
			$modules	=	$this->core->tendoo_admin->get_modules();
		}
		public function createWidget($title,$description,$contenu,$first_creation= FALSE)
		{
			$query	=	$this->core->db->where('WIDGET_HEAD',$title)->get('Tendoo_mod_widgets');
			...
			if(count($result) == 0)
			{
				...
				if($this->core->db->insert('Tendoo_mod_widgets',$array))
				{
		...
		{
			$query	=	$this->core->db->get('Tendoo_mod_widgets');
			$result	=	$query->result_array();
			return count($result);
		}
		...
		public function editWidget($widget_id,$widget_title,$widget_description,$widget_content)
		{
			...
			if($retreiver)
			{
				return $this->core->db->where('ID',$widget_id)->update('Tendoo_mod_widgets',array(
		...
		public function editSpecialWidget($widget_id,$widget_title,$widget_description,$widget_ref)
		{
			...
			if($retreiver)
			{
				...
				$module_namespace	=	$data[0];
				$widget_namespace	=	$data[1];
				$time				=	$this->core->tendoo->datetime();
				return $this->core->db->where('ID',$widget_id)->update('Tendoo_mod_widgets',array(
					'WIDGET_HEAD'						=>	$widget_title,
					'WIDGET_REFERING_OBJ_NAMESPACE'		=>	$module_namespace,
					'WIDGET_REFERING_NAME'				=>	$widget_namespace,
		...
		public function activateWidget($widget_id)
		{
			...
			if($widget)
			{
				if($this->core->db->where('ID',$widget_id)->update('Tendoo_mod_widgets',array('WIDGET_ETAT'=>1)))
				{
		...
		public function disableWidget($widget_id)
		{
			...
			if($widget)
			{
				if($this->core->db->where('ID',$widget_id)->update('Tendoo_mod_widgets',array('WIDGET_ETAT'=>0)))
				{
		...
		public function grapWidget($widget_id)
		{
			...
			if($w)
			{
				...
				{
					$theBefore	=	$this->core->db->where('WIDGET_ORDER',$w[0]['WIDGET_ORDER']-1)->get('Tendoo_mod_widgets');
					$result		=	$theBefore->result_array();
					
					$this->core->db->where('ID',$result[0]['ID'])->update('Tendoo_mod_widgets',array('WIDGET_ORDER'=>$w[0]['WIDGET_ORDER']));
					if($this->core->db->where('ID',$w[0]['ID'])->update('Tendoo_mod_widgets',array('WIDGET_ORDER'=>	$result[0]['WIDGET_ORDER'])))
					{
						return 'graped';
					}
					return 'error_occured';
		...
		public function deleteWidget($widget_id)
		{
			...
			if(count($widget) > 0)
			{
				if($this->core->db->where('ID',$widget_id)->delete('Tendoo_mod_widgets'))
				{
		...
		{
			// HERE
			$allMod	=	$this->core->tendoo_admin->get_modules($start = NULL,$end = NULL);
		}
	}
