...
// 	hubby has already beend declared
$(document).ready(function(){
	if(typeof hubby.app  ==  'undefined'){ // Pour évider le conflict d'un double appel via AJAX
		...
		hubby.notice		=	new function() {
			...
			if($(parent).length == 0)
			{
				$('body').append('<div id="appendNoticeHere" style="position:fixed;top:10px;right:10px;z-index:'+hubby.zIndex.modal+';width:20%;"/>');
			}
		...
		hubby.window		=	new function(){
			...
			var	ajaxBinder	=	function(currentId){
				...
				$('[data-modal-id="'+currentId+'"]').find('a[href]').each(function(){
					$(this).bind('click',function(){
						$.ajax({
							beforeSend	:	function(){
								hubby.loader.show($('#loadingStatus'));
							},
							success	:	function(data){
								hubby.loader.remove($('#loadingStatus'));
						...
						$(this).bind('submit',function(){
							...
							$.ajax({
								...
								success	:	function(returned){
									...
									{
										hubby.notice.alert('Le serveur a renvoy&eacute; un contenu vide','warning');
									}
									ajaxBinder(currentId);
			...
			this.show		=	function(e){
				...
				var currentE	=	'[data-modal-id="'+currentId+'"]';
				var modal	=	'<div data-modal-id="'+currentId+'" style="z-index:'+hubby.zIndex.window+';display:block;width:100%;height:100%;position:fixed;top:0;left:0;background:rgba(76,85,102,0.5);overflow:hidden">'+
				...
				$(document).ready(function(){
					...
					$(this).bind('click',function(){
						if(typeof $(this).attr('data-status') == 'undefined' || $(this).attr('data-status') == 'clickable')
						{
							...
							var loadingStatus	=	
							'<div id="loadingStatus" style="z-index:'+hubby.zIndex.loader+';position:fixed;right:15px;bottom:10px;">'+
							'</div>';
							var object;
							$.ajax({
								beforeSend		:	function(jqXHR, settings){
									...
									object	=	hubby.loader.show($('#loadingStatus'));
		...
		hubby.loader		=	new function(){
			this.show		=	function(x){
				...
				var cTotalFrames=32;
				var cFrameWidth=50;
				var cImageSrc=	hubby.url.main()+'hubby_assets/img/images/sprites.png';
