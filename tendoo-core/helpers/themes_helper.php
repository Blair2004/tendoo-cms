...
<?php
	...
	* declare_theme
	**/
	function declare_theme( $namespace , $config ){
		$declared_themes	=	( $declared_themes = get_core_vars( _get_app_declaration_prefix() . 'declared_themes' ) ) ? $declared_themes : array();
		$declared_themes[ $namespace ] =	$config;
		set_core_vars( _get_app_declaration_prefix() . 'temp_declared_theme' , $namespace );
		return set_core_vars( _get_app_declaration_prefix() . 'declared_themes' , $declared_themes );
	}
	function push_theme_sql( $namespace , $sql_query ){
		$old_queries	=	_get_declared_theme_values( $namespace , 'sql_queries' );
		$old_queries[]	=	$sql_query;
		return _set_declared_themes_values( $namespace , 'sql_queries' , $old_queries );
	}
	function _set_declared_themes_values( $namespace , $key , $value ){
		$declared_themes	=	( $declared_themes = get_core_vars( _get_app_declaration_prefix() . 'declared_themes' ) ) ? $declared_themes : array();
		if( riake( $namespace , $declared_themes ) ){
			$declared_themes[ $namespace ][ $key ] =	$value;
			return set_core_vars( _get_app_declaration_prefix() . 'declared_themes' , $declared_themes );
		}
		return false;		
	}
	function _get_declared_theme_values( $namespace , $key ){
		$declared_themes	=	( $declared_themes = get_core_vars( _get_app_declaration_prefix() . 'declared_themes' ) ) ? $declared_themes : array();
		if( riake( $namespace , $declared_themes ) ){
			return $declared_themes[ $namespace ][ $key ];
		}
		return false;		
	}
/***
...
function load_themes(){
	$module_dir	=	opendir( THEMES_DIR );
	while( FALSE !== ( $dir	=	readdir( $module_dir ) ) ){
		if( !in_array( $dir , array( '.' , '..' ) , true ) && is_dir( THEMES_DIR . $dir ) ){
			if( file_exists( $file	= THEMES_DIR . $dir . '/theme-config.php' ) ){
				include_once( $file );
				// Récupération du dernier thème chargé
				$lastest_declared	=	get_core_vars( 'temp_declared_theme' );
				$active_theme		=	is_array( $active_theme = get_meta( 'active_theme' ) ) ? $active_theme : array();
				$is_active			=	in_array( $lastest_declared , $active_theme );
				$icon				=	is_file(  $icon = THEMES_DIR . $dir . '/app_icon.png' ) ? $icon : 
										is_file(  $icon = THEMES_DIR . $dir . '/app_icon.jpg' ) ? $icon : "";
				_set_declared_themes_values( $lastest_declared , 'uri_path' , THEMES_DIR . $dir . '/' );
				_set_declared_themes_values( $lastest_declared , 'url_path' , get_instance()->url->main_url() . THEMES_DIR . $dir . '/' );
				_set_declared_themes_values( $lastest_declared , 'encrypted_dir' , $dir );
				_set_declared_themes_values( $lastest_declared , 'namespace' , $lastest_declared );
				_set_declared_themes_values( $lastest_declared , 'active' , $is_active );
			}
		}
	}
}
function get_themes( $filter = 'filter_namespace' , $namespace = null , $limit = null)
{
	$declared_themes	=	get_core_vars( 'declared_themes' );
	if( $filter == 'filter_active' )
	{
		if( is_array( $declared_themes ) ){
			foreach( $declared_themes as $_theme ){
				if( riake ( 'active' , $_theme ) == true ){
					return $_theme;
				}
			}
		}
...
*	"active_theme" est déclaré sur tendoo-core/controllers/admin.php ::__construct()
* 	"active_theme" est déclaré sur tendoo-core/Systeme.Core.php ::boot()
**/
...
function unactive_theme( $namespace ){
	$active_modules			=	is_array( $active_modules = get_meta( 'active_theme' ) ) ? $active_modules : array();
	if( $is_active			=	in_array( $namespace , $active_modules ) ){
		...
		return set_meta( 'active_theme' , $active_modules );
	}
	return false;
}
function active_theme( $namespace ){
	// Activate only if the theme exists
	if( $theme	= get_themes( 'filter_namespace' , $namespace ) ){
		$active_modules			=	is_array( $active_modules = get_meta( 'active_theme' ) ) ? $active_modules : array();
		if( FALSE === ( $is_active			=	in_array( $namespace , $active_modules ) ) ){
			$active_modules	=	array( $namespace );
			return set_meta( 'active_theme' , $active_modules );
		}
	}
	return false;
}
function theme_thumb( $namespace ){
	$access	=	get_instance()->url->main_url().ASSETS_DIR.'img/theme_default.png';
	$theme	=	get_themes( 'filter_namespace' , $namespace );
	if(is_file( $theme[ 'uri_path' ] . 'preview.png'))
	{
		$access	= get_instance()->url->main_url() . $theme['uri_path'].'preview.png';
	}
	else if(is_file( $theme[ 'uri_path' ] . 'preview.jpg'))
	{
		$access	= get_instance()->url->main_url(). $theme[ 'uri_path' ].'preview.jpg';
	}
	return $access;
}
