...
class Installation extends Libraries
{
	...
	public function createTables()
	{
		...
		*/
		...
		/// ADMIN REF_ROLE_ID TABLE create  `tendoo_permissions`
		$sql = 
		'CREATE TABLE IF NOT EXISTS `'.$DB_ROOT.'tendoo_permissions` (
		  `ID` int(11) NOT NULL AUTO_INCREMENT,
		  ...
		  `REF_ROLE_ID` varchar(11) NOT NULL,
		  PRIMARY KEY (`ID`)
		) ENGINE=InnoDB;';
		if(!$this->db->query($sql))
		{
	...
	public function app_step( $step )
	{
		...
		if( $step == 1 ){
			...
			if( $this->createTables() && get_instance()->meta_datas->set( 'site_name' , $this->input->post( 'site_name' ) , 'from_install_interface' , 'system' ) ){ 
				...
				// Creating Base Roles
				$this->tendoo_admin->create_role( 'Administrateur' , '' , 0 );
