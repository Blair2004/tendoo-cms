<?php
$filemanager_path	=	LIBRARIES_DIR . 'filemanager/';
$config = include LIBRARIES_DIR . 'filemanager/config/config.php';
...
if(isset($_POST['submit'])){
	...
	include get_instance()->url->site_url( array( 'admin' , 'send' ) );
}
else {
	
include $filemanager_path . 'include/utils.php';
...
<html xmlns="https://www.w3.org/1999/xhtml">
	<head>
		...
		<link href="<?php echo get_instance()->url->main_url() . $filemanager_path;?>css/style.css" rel="stylesheet" type="text/css" />
		<link href="<?php echo get_instance()->url->main_url() . $filemanager_path;?>js/jPlayer/skin/blue.monday/jplayer.blue.monday.css" rel="stylesheet" type="text/css">
	<!--[if lt IE 8]><style>
	...
	<script src="<?php echo get_instance()->url->main_url() . $filemanager_path . 'js/plugins.js';?>"></script>
	<script src="<?php echo get_instance()->url->main_url() . $filemanager_path;?>js/jPlayer/jquery.jplayer/jquery.jplayer.js"></script>
	<script src="<?php echo get_instance()->url->main_url() . $filemanager_path;?>js/modernizr.custom.js"></script>
    <script src="<?php echo get_instance()->url->main_url() . $filemanager_path;?>/js/feather.js "></script>
	<?php
	/**
	if ($aviary_active){
	...
	**/?>
	...
	<script>
		...
		<?php
	      ...
	      onSave: function(imageID, newURL) {
			    ...
			    $.ajax({
						type: "POST",
						url: "<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' , 'ajax_call' ) );?>?action=save_img",
	...
	<script src="<?php echo get_instance()->url->main_url() . $filemanager_path;?>js/include.js"></script>
</head>
<body>
	...
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
	    ...
	    <div class="tab-content">
			<div class="tab-pane active" id="tab1">
		    	<?php } ?>
				<form action="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' ) );?>" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
...
<!-- header div end -->
    ...
    <div class="row-fluid">
	<?php
	$link= get_instance()->url->site_url( array( 'admin' , 'upload' ) ) . "?" . $get_params;
	?>
	<ul class="breadcrumb">
	...
	<li class="pull-right"><a id="refresh" class="btn-small" href="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' ) ) . "?";?><?php echo $get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			...
			if (!file_exists($thumbs_path.$subdir.$file)) //var_dump( $subdir );// create_folder(false,$thumbs_path.$subdir.$file);
			$class_ext = 3;
			if($file=='..' && trim($subdir) != '' ){
		    $src = explode("/",$subdir);
			...
			?>
			    ...
			    <?php } ?>
				  <a class="folder-link" href="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' ) ) . "?";?><?php echo $get_params.rawurlencode($src)."&".uniqid() ?>">
					  <div class="img-precontainer">
							<div class="img-container directory"><span></span>
							<img class="directory-img"  src="<?php echo get_instance()->url->main_url() . $filemanager_path;?>img/<?php echo $icon_theme; ?>/folder<?php if($file==".."){ echo "_back"; }?>.png" />
							</div>
					  </div>
			...
			<?php }else{ ?>
				    ...
				    <div class="box">
					<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' ) ) . "?";?><?php echo $get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file; ?></a></h4>
				    </div>
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<div class="img-precontainer">
				    ...
				    <div class="img-container">
					    <span></span>
					    <img class="<?php echo $show_original ? "original" : "" ?><?php echo $is_icon_thumb ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo get_instance()->url->main_url() . $src_thumb; ?>">
				    </div>
				</div>
				<div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
					...
					<span></span>
					<?php if($mini_src!=""){ ?>
					<img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo get_instance()->url->main_url() . $mini_src; ?>">
					<?php } ?>
				    </div>
				</div>
				...
				<figcaption>
				    <form action="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' , 'force_download' ) );?>" method="post" class="download-form" id="form<?php echo $nu; ?>">
					...
					title="<?php echo trans('Preview')?>" data-url="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' , 'ajax_call' ) );?>?action=media_preview&title=<?php echo $filename; ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file; ?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					    ...
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' , 'ajax_call' ) );?>?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename; ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file; ?>"
						href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
						<?php }elseif($googledoc_enabled && in_array($extension_lower,$googledoc_file_exts)){ ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' , 'ajax_call' ) );?>?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename; ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file; ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

						<?php }elseif($viewerjs_enabled && in_array($extension_lower,$viewerjs_file_exts)){ ?>
					    <a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="<?php echo get_instance()->url->site_url( array( 'admin' , 'upload' , 'ajax_call' ) );?>?action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename; ?>&file=<?php echo $current_path.$rfm_subfolder.$subdir.$file; ?>"
						href="docs.google.com;" ><i class=" icon-eye-open"></i></a>
...
<script>
    ...
    <div id="loading_container" style="display:none;">
	    ...
	    <img id="loading_animation" src="<?php echo get_instance()->url->main_url() . $filemanager_path;?>img/storing_animation.gif" alt="loading" style="z-index:10001; margin-left:-32px; margin-top:-32px; position:fixed; left:50%; top:50%"/>
    </div>
    <!-- loading div end -->
