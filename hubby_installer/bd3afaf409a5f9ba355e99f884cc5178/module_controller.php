<?php
class Hubby_widget_administrator_module_controller
{
	protected 	$data;
	private 	$news;
	private 	$core;
	...
	public function __construct($data)
	{
		...
		$this->data['widgetHandler']=		new widhandler_common($this->data);
		$this->data['userUtil']		=&		$this->core->users_global;		
		$this->data['ttActivWid']	=		$this->data['widgetHandler']->getWidgets();
		if(count($this->data['ttActivWid']) > 0 && is_array($this->data['ttActivWid']))
		{
			foreach($this->data['ttActivWid'] as $w) // Récupération des widgets activé depuis l'espace d'admin (ESAD)
			{
				...
				{
					$module			=	$this->core->hubby->getSpeMod($w['WIDGET_REFERING_OBJ_NAMESPACE'],FALSE); // Recuperation du module en question
					...
					if(is_file($configFile)) // Test sur l'exitence des fichiers.
					{
						...
						if(array_key_exists($w['WIDGET_REFERING_NAME'],$WIDGET_CONFIG)) // verification sur l'existence de la clé porteuse d'un tableau
						{
							...
							$w_output	=	$this->core->hubby->interpreter(
