...
/// -------------------------------------------------------------------------------------------------------------------///
$or['unknowContactMessage']			=	hubby_error('Discussion introuvable ou indisponible.');

/// -------------------------------------------------------------------------------------------------------------------///
$NOTICE_SUPER_ARRAY = $or;
/// -------------------------------------------------------------------------------------------------------------------///

class hubby_contact_handler_admin_controller
{
	public function __construct($data)
	{
		...
		if($this->hubby_admin->actionAccess('hubby_contact_handler','hubby_contact_handler') === FALSE)
		{
			$this->url->redirect(array('admin','index?notice=accessDenied'));
		}
		include_once($this->module_dir.'/library.php');
		$this->lib						=	new hubby_contact_handler_class();
		$this->hubby_admin->menuExtendsBefore($this->load->view($this->module_dir.'/views/menu',$this->data,true,TRUE));
	...
	public function index($page	=	1,$action = "",$element	=	'')
	{
		...
		$this->data['paginate']			=	$this->hubby->paginate(10,$this->data['countPost'],1,'','',$page,$this->url->site_url(array('admin','open','modules',$this->module_id,'index')).'/',$ajaxis_link=null);
		...
		$this->hubby->setTitle($this->data['module'][0]['HUMAN_NAME'].' - Liste des messages');
	...
	public function setting()
	{
		...
		$this->hubby->setTitle($this->data['module'][0]['HUMAN_NAME'].' - Param&ecirc;tres');
	...
	public function check($id)
	{
		...
		$this->hubby->setTitle($this->data['module'][0]['HUMAN_NAME'].' - Lire un message');
